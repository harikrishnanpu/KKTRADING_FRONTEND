...
export default function ReturnListingScreen() {
  ...
  // Function to generate PDF of a specific return via backend API
  const generateReturnPDF = async (returnEntry) => {
    setPdfLoading(true);
    try {
  ...
  const paginateReturns = () => {
    ...
    return returns.slice(start, start + itemsPerPage);
  };
              ...
              {returns.length === 0 ? (
                ...
                <>
                    ...
                    <table className="w-full text-xs text-gray-500 bg-white shadow-md rounded-lg overflow-hidden">
                      ...
                      <tbody>
                        {paginateReturns().map((returnEntry) => (
                          ...
                          >
                            <td className="px-2 py-2">
                              <span className={`flex text-xs font-bold text-red-600 cursor-pointer`}>
                                {returnEntry.returnNo} 
                                {returnEntry.isApproved && <img className='h-4 w-4 ml-1 mt-1' src='/images/tick.svg' alt="Approved" />}
                              </span>
                            </td>
                            <td className="px-2 py-2">
                            ...
                            <td className="px-2 py-2">
                              {returnEntry.billingNo}
                            </td>
                            <td className="px-2 py-2">
                              {returnEntry.customerName}
                            </td>
                            <td className="px-2 py-2">
                              {returnEntry.products.length}
                            </td>
                            <td className="px-2 py-2">
                              {returnEntry.netReturnAmount}
                            </td>
                            <td className="px-2 py-2">
                              <div className="flex space-x-2">
                                ...
                                <button
                                  onClick={() => navigate(`/returns/edit/${returnEntry.returnNo}`)}
                                  className={`bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 flex items-center ${
                                    !userInfo.isAdmin && returnEntry.isApproved ? 'bg-gray-300 cursor-not-allowed' : ''
                                  }`}
                  ...
                  <div className="md:hidden space-y-4">
                    {paginateReturns().map((returnEntry) => (
                      <div key={returnEntry.returnNo} className="bg-white rounded-lg shadow-md p-6 mb-4">
                        <div className="flex justify-between items-center">
                          <p className={`text-xs font-bold text-red-600`}>
                            Return No: {returnEntry.returnNo}
                            {returnEntry.isApproved && <img className='h-4 w-4 ml-1 mt-1' src='/images/tick.svg' alt="Approved" />}
                          </p>
                          {renderStatusIndicator(returnEntry)}
                        </div>
                        <p className="text-gray-600 text-xs mt-2">Return Date: {new Date(returnEntry.returnDate).toLocaleDateString()}</p>
                        <p className="text-gray-600 text-xs mt-1">Billing No: {returnEntry.billingNo}</p>
                        <p className="text-gray-600 text-xs mt-1">Customer: {returnEntry.customerName}</p>
                        <div className="mt-4">
                          <p className="text-xs font-bold">Total Products: {returnEntry.products.length}</p>
                        </div>
